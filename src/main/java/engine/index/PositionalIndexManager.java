...
public class PositionalIndexManager extends InvertedIndexManager {
    ...
    /**
     * write one page by the ByteBuffer
     *
     * @param pfc: page file channel
     * @param bb: the byte buffer
     * @param l: the list
     * @param map: map of integer and integer
     * @param offsetPos: positional offset
     * @return an integer as the offset
     */
...
//        System.out.println("merge:"+segID1+" and "+segID2);
    ...
    /*private Map<Integer, List<Integer>> findWord(PageFileChannel pfc, String target, int segID) {
        ...
        while (true) {
            ...
            // the length of the dictionary word
            try {
            ...
            // length of the bytes of the posting list
            try {
                lengthPosting = bb.getInt();
            }
            catch (BufferUnderflowException e) {
                pageNumber++;
                if (pageNumber >= cap) {
            ...
            // offset of the positional list for each docID
            try {
                offsetPosition = bb.getInt();
            }
            catch (BufferUnderflowException e) {
                pageNumber++;
                if (pageNumber >= cap) {
            ...
            // length of the bytes of the positional list
            try {
                lengthPosition = bb.getInt();
            }
            catch (BufferUnderflowException e) {
                pageNumber++;
                if (pageNumber >= cap) {
            ...
            if (dictWord.equals(target)) {
                idList = getPostingList(segID, offsetPosting, lengthPosting);
                posList = getPositionalList(segID, offsetPosition, lengthPosition);
                wordList = makeMap(idList, posList);
                break;
            }
        }
        return wordList;
    ...
    private Map<Integer, List<Integer>> makeMap(List<Integer> id, List<List<Integer>> pos) { // TODO: change, study
        return new HashMap<>();
    }
    ...
    /**
     * Get the inverted list (posting list) in the segment with given offset and length.
     *
    ...
    private List<Integer> getPostingList(int segID, int offset, int length) {
        ...
        int cap = pfc.getNumPages();
        if (pageID >= cap) {
            System.err.println("offset in segment" + segID + "b is incorrect");
            System.exit(-1);
        }
        ByteBuffer idBuffer = pfc.readPage(pageID);
        idBuffer.position(offset);
        ...
        pfc.close();
        List<Integer> idList = compressor.decode(byteList);
        return idList;
    }
    ...
    private List<Integer> getPositionalOffsets(int segID, int offset, int length) { // TODO: change later
    ...
    private List<Integer> getPositionalList(int segID, int offset, int length) { // TODO: to change later
        List<Integer> list = new ArrayList<>();
        Path path = Paths.get(indexFolder + "/segment" + segID + "c");
        PageFileChannel pfc = PageFileChannel.createOrOpen(path);
        int cap = pfc.getNumPages();
